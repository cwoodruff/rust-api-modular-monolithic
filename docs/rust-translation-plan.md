...
| F14 | Every `traceId` is minted at the moment a problem document is built, so the identifier in the body matches no log record and nothing a client can quote. The one field whose purpose is correlation correlates with nothing | One identifier per request: the ambient value every document reads, a field on the span wrapping the request, and an `x-trace-id` response header. An inbound `traceparent` continues the trace when it really is one |
